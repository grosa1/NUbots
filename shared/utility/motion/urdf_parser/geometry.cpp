    ...
    if (xml->Attribute("radius") != nullptr) {
        try {
            ...
            error_msg << "Error while parsing link '" << getParentLinkName(xml) << "': sphere radius ["
                      << xml->Attribute("radius") << "] is not a valid float: " << e.what() << "!";
    ...
    if (xml->Attribute("length") != nullptr && xml->Attribute("radius") != nullptr) {
        try {
            ...
            error_msg << "Error while parsing link '" << getParentLinkName(xml) << "': cylinder length ["
                      << xml->Attribute("length") << "] is not a valid float: " << e.what() << "!";
            throw URDFParseError(error_msg.str());
        }
...
std::shared_ptr<Geometry> Geometry::fromXml(TiXmlElement* xml) {
    ...
    if (type_name == "sphere") {
        ...
        std::ostringstream error_msg;
        error_msg << "Error while parsing link '" << getParentLinkName(xml) << "' unknown shape type '" << type_name
                  << "'!";
        throw URDFParseError(error_msg.str());
    }
}
