...
Vector3 Vector3::fromVecStr(const string& vector_str) {
    ...
    if (values.size() != 3) {
        ...
        error_msg << "Parser found " << values.size() << " elements but 3 expected while parsing vector [" << vector_str
                  << "]";
    ...
    for (int i = 0; i < pieces.size(); i++) {
        if (!pieces[i].empty()) {
            try {
                ...
                error_msg << "Error parsing Color value " << i << " in color value string (" << vector_str
